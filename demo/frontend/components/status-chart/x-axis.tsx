...
import { cn } from "@/components/status-chart/utils";
...
function dedupeIndicesByLabel(
  ...
  for (const index of indices) {
    ...
    const label = dateLabels[index] ?? hmsTimeFmt.format(xAccessor(point));
...
function buildDataAlignedTicks({
  ...
  for (const index of selectEvenlySpacedIndices(data.length, targetTickCount, {
    ...
    const label = dateLabels[index] ?? hmsTimeFmt.format(date);
    if (seenLabels.has(label)) {
      continue;
    }
    seenLabels.add(label);
...
function buildDomainTicks({
  ...
  for (let i = 0; i < tickCount; i++) {
    ...
    const label = hmsTimeFmt.format(date);
    if (seenLabels.has(label)) {
      continue;
    }
    seenLabels.add(label);
...
function appendProjectionTailTicks(
  ...
  const endLabel = hmsTimeFmt.format(domainEnd);
  ...
  const hoveredLabel =
    isHovering && tooltipData
      ? (dateLabels[tooltipData.index] ??
        hmsTimeFmt.format(xAccessor(tooltipData.point)))
      : null;
